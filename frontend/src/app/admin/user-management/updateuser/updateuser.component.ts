  ...
  onSubmit(customerData) {
    ...
    if (customerData) {
      if (this.userID === 'new') {
        ...
        customerData.postid = this.userID;
  ...
  processFile(postid) {
    if (this.inputImg.nativeElement.files.length > 0) {
      ...
      reader.addEventListener('load', (event: any) => {
        ...
        this.adminService.uploadImage(postid, this.selectedFile.file).subscribe(
          (res) => {
            console.log(res);
            this.onSuccess();
          },
  ...
  onUserSelected() {
    if (this.userID !== 'new') {
      const userData = this.persons.find((person) => {
        return String(person.postid) === String(this.userID);
      });
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result === 'true') {
        const username = this.persons.find((person) => {
          return String(person.postid) === this.userID;
        });
